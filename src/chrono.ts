...
export class Chrono {
  ...
  parseDate(
    ...
    return results[0]?.start.date() ?? undefined;
  }
  ...
  private static executeParser(context: ParsingContext, parser: Parser) {
    ...
    while (match) {
      ...
      if (!result) {
        ...
        remainingText = originalText.slice(Math.max(0, match.index + 1));
      ...
      remainingText = originalText.slice(
        Math.max(0, parsedIndex + parsedText.length)
      );
      match = pattern.exec(remainingText);
    }
...
export class ParsingContext implements DebugHandler {
  ...
  constructor(
    ...
    this.text = text;
    this.reference = new ReferenceWithTimezone(referenceDate);
    this.option = option ?? {};
  ...
  createParsingResult(
    ...
    const text =
      typeof textOrEndIndex === "string"
        ? textOrEndIndex
        : this.text.slice(index, textOrEndIndex);
