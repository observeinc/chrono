...
export function getNthWeekdayOfMonth(
  ...
  while (index < n) {
    ...
    if (date.getDay() === weekday.valueOf()) index++;
  }
...
export function getLastWeekdayOfMonth(
  ...
  const oneIndexedWeekday = weekday.valueOf() === 0 ? 7 : weekday;
  const date = new Date(year, month - 1 + 1, 1, 12);
  const firstWeekdayNextMonth = date.getDay() === 0 ? 7 : date.getDay();
  let dayDiff;
  if (firstWeekdayNextMonth === oneIndexedWeekday.valueOf()) dayDiff = 7;
  else if (firstWeekdayNextMonth < oneIndexedWeekday.valueOf())
...
export function toTimezoneOffset(
  ...
  if (
    DateTime.fromJSDate(date) >
      DateTime.fromJSDate(matchedTimezone.dstStart(date.getFullYear())) &&
    !(
      DateTime.fromJSDate(date) >
      DateTime.fromJSDate(matchedTimezone.dstEnd(date.getFullYear()))
    )
  ) {
