...
test("Test - Date + Time Expression", function () {
  testSingleCase(
    chrono,
    "Something happen on 2014-04-18 13:00 - 16:00 as",
    (result) => {
...
test("Test - Quoted Expressions", function () {
  testSingleCase(
    chrono,
    "Want to meet for dinner (5pm EST)?",
    new Date(2020, 7 - 1, 6),
    (result) => {
...
test("Test - Random text", function () {
  ...
  testSingleCase(
    chrono,
    "...Thursday, December 15, 2011 Best Available Rate ",
    (result) => {
  ...
  testSingleCase(
    chrono,
    "9:00 AM to 5:00 PM, Tuesday, 20 May 2013",
    (result) => {
  ...
  testSingleCase(
    chrono,
    "Monday afternoon to last night",
    new Date(2017, 7 - 1, 7),
    (result) => {
...
test("Test - Customize by removing time extraction", () => {
  ...
  custom.parsers = custom.parsers.filter(
    (p) => !(p instanceof ENTimeExpressionParser)
  );
