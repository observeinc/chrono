...
test("Test - Single Expression", () => {
  ...
  testSingleCase(
    chrono.casual,
    "The Deadline was this afternoon ",
    new Date(2012, 7, 10, 12),
    (result) => {
      expect(result.index).toBe(17);
...
test("Test - Casual date range", () => {
  testSingleCase(
    chrono.casual,
    "The event is today - next friday",
    new Date(2012, 7, 4, 12),
    (result) => {
...
test("Test - Casual time with timezone", () => {
  ...
  testSingleCase(chrono, "Jan 1, 2020 Evening JST", (result, text) => {
    ...
    expect(result).toBeDate(
      new Date("Wed Jan 01 2020 20:00:00 GMT+0900 (Japan Standard Time)")
    );
  });
});
