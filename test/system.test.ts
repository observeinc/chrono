...
test("Test - Basic parse date functions", function () {
  expect(chrono.parseDate("7:00PM July 5th, 2020")).toStrictEqual(
    new Date(2020, 7 - 1, 5, 19)
  );

  expect(chrono.en.parseDate("7:00PM July 5th, 2020")).toStrictEqual(
    new Date(2020, 7 - 1, 5, 19)
  );

  expect(chrono.strict.parseDate("7:00PM July 5th, 2020")).toStrictEqual(
    new Date(2020, 7 - 1, 5, 19)
  );

  expect(chrono.casual.parseDate("7:00PM July 5th, 2020")).toStrictEqual(
    new Date(2020, 7 - 1, 5, 19)
  );
});

test("Test - Add custom parser", () => {
  const customParser: chrono.Parser = {
    pattern: () => {
...
test("Test - Remove a parser example", () => {
  ...
  custom.parsers = custom.parsers.filter(
    (r) => !(r instanceof SlashDateFormatParser)
  );
...
test("Test - Remove a refiner example", () => {
  const custom = chrono.casual.clone();
  custom.refiners = custom.refiners.filter(
    (r) => !(r instanceof UnlikelyFormatFilter)
  );
...
test("Test - Replace a parser example", () => {
  ...
  const index = custom.parsers.findIndex(
    (r) => r instanceof ENTimeUnitCasualRelativeFormatParser
  );
